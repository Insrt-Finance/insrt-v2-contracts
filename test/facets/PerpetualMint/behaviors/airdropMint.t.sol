...
/// @title PerpetualMint_airdropMint
/// @dev PerpetualMint test contract for testing expected airdropMint behavior. Tested on an Arbitrum fork.
contract PerpetualMint_airdropMint is
    ArbForkTest,
    ...
    TokenTest
{
    ...
    /// @dev Tests airdropMint functionality mints AIRDROP_AMOUNT of tokens
    ...
    /// @dev tests that airdropMint increases consolation fees by ETH needed for airdrop
    function test_airdropMintIncreasesConsolationFees() external {
        uint256 ethRequired = AIRDROP_AMOUNT / perpetualMint.ethToMintRatio();
    ...
    /// @dev tests that airdropMint reverts when incorrect amount of ETH is received
    function test_airdropMintRevertsWhen_IncorrectAmountOfETHIsSent() external {
        uint256 ethRequired = AIRDROP_AMOUNT / perpetualMint.ethToMintRatio();

        vm.expectRevert(IPerpetualMintInternal.IncorrectETHReceived.selector);

        perpetualMint.airdropMint{ value: ethRequired - 10 }(AIRDROP_AMOUNT);
    }

    /// @dev tests that airdropMint revents when caller is not owner
    function test_airdropMintRevertsWhen_CallerIsNotOwner() external {
        uint256 ethRequired = AIRDROP_AMOUNT / perpetualMint.ethToMintRatio();
